...
{
  if(EXTI->PR & EXTI_PR_PR8) // Check if interrupt was triggered by pin PA8
  {
    EXTI->PR |= EXTI_PR_PR8; // Clear interrupt flag
    // Handle the interrupt here
    snprintf(msgBuf, MSGBUFSIZE, "%s", "Interrupt geactiveerd!\r\n");
...
int main(void)
{
  ...
  /////////////////////////////////////
  /// Het aanzetten van de interrupts//
  //////////////////////////////////////

  //In het RCC_APB2ENR register moet eerst de Sysfgen bit (de clock) worden aangezet om interrupts te laten werken
  RCC->APB2ENR = (RCC->APB2ENR & ~RCC_APB2ENR_SYSCFGEN) | (0b01 & RCC_APB2ENR_SYSCFGEN_Pos);

  //dan vervolgens zet ik PINA8 in als interrupt pin dit doe ik in het SYSCFG_EXTICR3 register 
  SYSCFG->EXTICR[2] = (SYSCFG->EXTICR[2] & ~SYSCFG_EXTICR3_EXTI8) | (0b000 & SYSCFG_EXTICR3_EXTI8_Pos);
  //SYSCFG->EXTICR[2] |= SYSCFG_EXTICR3_EXTI8_PA;

  EXTI->FTSR |= EXTI_FTSR_TR8; // Set interrupt EXTI0 trigger to falling edge
  //EXTI->IMR |= EXTI_IMR_IM8;     // Unmask EXTI8 line
  NVIC_EnableIRQ(EXTI9_5_IRQn); // Enable EXTI0_1 interrupt
 
  while (1)
  {
    //het aflezen van de button pin en dit vervolgens uiprinten
    buttonstate = (GPIOA->IDR & GPIO_IDR_8) >> 8; //hij moet 8 na rechts worden verplaatst omdat het anders als 256 wordt gelezen (100000000)
