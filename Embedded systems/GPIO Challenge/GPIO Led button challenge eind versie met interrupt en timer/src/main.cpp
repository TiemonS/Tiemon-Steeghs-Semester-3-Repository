...
{
    EXTI->PR = EXTI_PR_PR0;   // Reset interrupt flag of EXTI0. This is needed so that the 		       // next interrupts are executed
...
int main(void)
{
  ...
  while (1)
  {
    ...
    // HAL_UART_Transmit(&huart2, (uint8_t *)msgBuf, strlen(msgBuf), HAL_MAX_DELAY);
    // }
    // else if (ButtonPressedTime > 20 && ButtonPressedTime < 1000)
    // {
    //   KnipperLEDPB5(500);
